...
function signText(text, caOption, misc) {
	...
	if (caOption == "auto" || caOption == "auto non-repudiation") {
		let uri = NetUtil.newURI(misc.location);
	...
	try {
		...
		for (;;) {
			...
			if (autoSign) {
				...
				selectedCert = selectCert(userCerts, text, misc.hostname);
		...
		let digestBytes = hash(text, misc.characterSet);
		let byteArray = ctypes.ArrayType(ctypes.uint8_t);
...
/*****************************************************************************
load the content script PageMod_ContentScriptFile.js on every page
	see
		https://developer.mozilla.org/en-US/Add-ons/SDK/High-Level_APIs/page-mod
*****************************************************************************/
pageMod.PageMod({
	include: "*",
	contentScriptFile: data.url("PageMod_ContentScriptFile.js"),
	...
	onAttach: function (worker)
	{
		...
		worker.port.emit("signText_msg_InitContentScript");


		/*****************************************************************************
		receive message from content-script
			call signText function
			send result message to content-script
		*****************************************************************************/
		worker.port.on("signText_msg_ContentCallsSignText", function (obj) 
		{
			var res = signText(obj.stringToSign, obj.caOption, obj.misc);
			worker.port.emit("signText_msg_AddonReturnsSignTextResult", res);
		});
